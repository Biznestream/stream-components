...
export function toRange (range) {
  ...
  if (range[0]) {
    res.min = parseInt(range[0]);
  }
  if (range[1]) {
    res.max = parseInt(range[1]);
  }
 ...
 * @returns {{}}
 */
export function qsToFilter (values, attributes, defaults, withoutDefaults) {
  var filter = {};
  values = { ...values };
  (attributes || []).forEach((attribute, n) => {
    ...
    var value = values[FILTER_PREFIX + attribute.name.toLowerCase()];
    delete values[FILTER_PREFIX + attribute.name.toLowerCase()];
 ...
 * @returns {string}
 */
export function filterToParams (pvalues, attributes, defaults, withDefault) {
  var params = {};
  let values = { ...pvalues };
  (attributes || []).forEach((attribute, n) => {
    if (attribute.hidden) {
    ...
    params[FILTER_PREFIX + attribute.name.toLowerCase()] = value;
  });

  Object.entries(values).forEach(([value, key]) => {
    params[FILTER_PREFIX + key.toLowerCase()] = value;
  });
  return params;
}

export function filterToQs (values, attributes, defaults) {
  var qs = [],
